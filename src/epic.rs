...
use crate::{epic_clients::AuthClient, epic_manifest_api::{EpicManifestResponse, EPIC_MANIFEST_API}, get_client, launcher::RememberMeEntry};
...
impl EpicAccount {
    ...
    pub async fn get_manifest(&self) -> Result<EpicManifestResponse, EpicError> {
        let response = CLIENT
            .get(EPIC_MANIFEST_API)
            .bearer_auth(&self.access_token)
            .send()
            .await
            .map_err(|_| EpicError::reqwest_internal_error())?;
        ...
        let data = response.json::<EpicManifestResponse>().await.map_err(|_| EpicError::new(EpicErrorKind::ParsingError, Some("Failed to parse JSON data")))?;
